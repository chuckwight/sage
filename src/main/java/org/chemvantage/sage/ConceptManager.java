...
public class ConceptManager extends HttpServlet {
		...
		try {
			...
			case "Save":
				try {
					...
					c = new Concept(conceptId,request.getParameter("Title"),request.getParameter("OrderBy"),chapterId,request.getParameter("Summary"));
					ofy().save().entity(c).now();
				}
				break;
			}
	...
	static String viewConcepts(HttpServletRequest request) {
		...
		buf.append("<form method=post>"  // build a Concept editing form
				+ "<table>"
				+ "<tr><td style='text-align:right;'>Title: </td><td><input size=25 name=Title value='" + (c1==null?"":c1.title) + "' />"
						+ "&nbsp;ConceptId:&nbsp;<input type=text name=ConceptId value='" + (c1==null?"":c1.id) + "' /></td><tr/>"
				+ "<tr><td style='text-align:right;'>OrderBy: </td><td><input size=10 name=OrderBy value='" + (c1==null?"":c1.orderBy) + "' />"
						+ "&nbsp;ChapterId:&nbsp;<input type=text name=ChapterId value='" + (c1==null?"":c1.chapterId) + "' /></td></tr>"
